...
generateBtn.addEventListener("click", () => {
  let inputLines = parseInt(document.getElementById("lines").value);
  if (isNaN(inputLines) || inputLines < 1 || inputLines > text.length) {
    textDisplay.innerHTML = `<p>Please enter a valid number between 1 and ${text.length}.</p>`;
    return;
  }
  ...
  textDisplay.innerHTML = selectedTexts
    .map((line) => `<p>${line}</p>`)
    .join("");
});
